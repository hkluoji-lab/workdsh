...
`WORKDSH_MAC_ARCH=arm64 yarn dist:mac-smoke` builds an unsigned Apple Silicon DMG; use `WORKDSH_MAC_ARCH=x64` for a separate Intel DMG. CI builds and checks both architectures independently, verifying the selected executable and native modules inside each mounted DMG. The package gate, signing-secret isolation, and headless checks remain the same. These CI artifacts are unsigned previews and may be blocked by Gatekeeper. On a credentialed Mac, `WORKDSH_MAC_ARCH=arm64 yarn dist:mac` or `WORKDSH_MAC_ARCH=x64 yarn dist:mac` builds the corresponding signed and notarized installer in `dsh-plugin-desktop-beta/dist/mac-release/<arch>/`.

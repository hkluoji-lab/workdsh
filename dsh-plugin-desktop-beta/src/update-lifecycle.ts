...
function updateAvailableNotification(locale: DesktopLocale, version: string): DesktopNotification {
  return locale === 'zh'
    ? { title: 'WorkDSH 有可用更新', body: `版本 ${version} 已可下载。打开 WorkDSH 即可继续。` }
    : { title: 'WorkDSH Update Available', body: `Version ${version} is ready to download. Open WorkDSH to continue.` }
}

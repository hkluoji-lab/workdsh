...
export const name = 'desktop-ssh'
export const inject = ['webServer', 'connection', 'llm']
const ROOT = '/desktop/ssh'
const MAX_BODY = 128 * 1024
/** Additional local-only fence: no SSH proxy exposed when Desktop enables LAN browsing. */
...
/** Built-in Host plugin using the existing authenticated carrier. */
export function apply(ctx: Context): void {
  ...
  ctx.effect(() => () => { for (const call of aiCalls.values()) call.abort(); service.dispose(); for (const client of sockets.clients) client.terminate(); sockets.close() }, 'desktop: SSH generation cleanup')
  ctx.effect(() => ctx.webServer.register({ kind: 'prefix', path: ROOT, handler: async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && (url.pathname === ROOT || url.pathname === ROOT + '/')) {
        ...
        res.end(await readFile(new URL('./native-ui/ssh.html', import.meta.url))); return
      }
      if (req.method === 'GET' && url.pathname.startsWith(ROOT + '/assets/')) {
        ...
        res.end(await readFile(new URL('./native-ui/assets/' + file, import.meta.url))); return
      }
      ...
      if (url.pathname === ROOT + '/connect') {
        const s = await service.connect(data as unknown as SshCredentials)
        let path: string

/** Isolated command-line environment launched from the DSH SSH tray. */
...
/** Platforms with a native terminal launch contract owned by DSH SSH. */
...
function macWelcome(
  ...
  return [
    ...
    `printf '%s\\n' ${quoteSh(`DSH SSH ${options.productVersion} terminal`)}`,
    ...
    `printf '%s\\n' ${quoteSh('Restart DSH SSH after plugin changes.')}`,
...
function windowsWelcome(): string {
  ...
  return [
    ...
    `Write-Host ("DSH SSH {0} terminal" -f $env:${WINDOWS_PRODUCT_VERSION})`,
    ...
    `Write-Host 'Restart DSH SSH after plugin changes.'`,
    '',
  ].join('\r\n')
}
...
function windowsCmdWelcome(): string {
  ...
  return [
    ...
    `echo(DSH SSH !${WINDOWS_PRODUCT_VERSION}! terminal`,
    ...
    `echo(${escapeBatchText('Restart DSH SSH after plugin changes.')}`,
...
function windowsLaunchBroker(
  ...
  return [
    '@echo off',
    'setlocal EnableDelayedExpansion',
    `start "DSH SSH Beta" /D "!${WINDOWS_PROFILE_DIRECTORY}!" ${target}`,
    'exit /b %errorlevel%',
    '',
  ].join('\r\n')
}

...
const COPY: Record<DesktopLocale, DesktopRecoveryCopy> = {
  en: {
    title: 'DSH SSH Recovery Assistant',
    fallbackBody: 'The recovery information could not be read. Quit and start DSH SSH again.',
    ...
    confirmSafeModeBody: 'DSH SSH will create an isolated DSH home without reading your normal DSH data directory. Your normal Profiles and data are not changed. The temporary environment is deleted on the next normal launch after you leave Safe Mode.',
    ...
    configurationFilesBody: 'View or edit the current Profile and shared Harness-home configuration. Restart DSH SSH after making changes.',
    ...
    confirmDataDirectoryChangeBody: 'If the target folder is empty, DSH SSH will create a new environment. The old data directory will not be deleted.',
    ...
    confirmRestoreDefaultDirectoryBody: 'DSH SSH will use the default data directory for this system after restart. The current data directory will not be deleted.',
    ...
    confirmCreateDefaultDirectoryBody: 'DSH SSH will create a new environment at the default path and use it after restart. The current data directory will not be deleted.',
    confirmCreateDefaultDirectoryAction: 'Create and restart',
    confirmFactoryReset: 'Factory reset DSH SSH?',
    confirmFactoryResetMessage: 'Remove all data in the current DSH data directory and reinstall?',
    confirmFactoryResetBody: currentDirectory => `The following directory will be moved to the operating-system Trash or Recycle Bin:\n\n${currentDirectory}\n\nProfiles, plugins, settings, credentials, sessions, attachments, and workspace records stored there will be removed from DSH SSH. Project files outside this directory are not deleted. DSH SSH will restart and create a clean default Profile.`,
    ...
    confirmRollbackBody: capturedAt => `This immediately restores the current Profile plus the checkpointed settings.yaml and Harness-home patch captured at ${capturedAt}. After restarting, DSH SSH will use the rolled-back configuration.`,
    confirmRollbackAction: 'Roll Back',
    uninstalledSuccess: 'The plugin was removed from the current Profile. Restart DSH SSH to use the updated plugin configuration.',
    rollbackSuccess: slotId => `Rolled back to ${slotId}. Restart DSH SSH to use this configuration; the first healthy start after rollback will preserve all three existing slots.`,
    profileSelectedSuccess: 'This Profile is now selected. Restart DSH SSH to use it.',
  ...
  zh: {
    title: 'DSH SSH 恢复助手',
    fallbackBody: '无法读取恢复信息。请退出并重新启动 DSH SSH。',
    reason: '进入恢复模式的原因',
    ...
    confirmSafeModeBody: 'DSH SSH 将创建一个不读取正常 DSH 数据目录的独立 DSH Home，不会修改原有 Profile 和数据。退出安全模式后，下一次普通启动会自动删除临时环境。',
    ...
    configurationFilesBody: '查看或编辑当前 Profile 与 DSH home 的共享配置。修改后需要重新启动 DSH SSH。',
    ...
    confirmDataDirectoryChangeBody: '目标文件夹如果为空，则DSH SSH 会创建一个全新的环境；旧数据目录不会被删除。',
    ...
    confirmRestoreDefaultDirectoryBody: 'DSH SSH 将改为使用当前系统的默认数据目录。当前数据目录不会被删除。',
    ...
    confirmCreateDefaultDirectoryBody: 'DSH SSH 将在默认路径创建一个全新的环境并重启。当前数据目录不会被删除。',
    confirmCreateDefaultDirectoryAction: '新建并重启',
    confirmFactoryReset: '恢复 DSH SSH 出厂设置？',
    confirmFactoryResetMessage: '删除当前 DSH 数据目录中的全部数据并重装？',
    confirmFactoryResetBody: currentDirectory => `以下目录将被移入系统废纸篓或回收站：\n\n${currentDirectory}\n\n其中保存的 Profile、插件、设置、凭据、会话、附件和工作区记录都会从 DSH SSH 中移除；不会删除此目录以外的项目文件。随后 DSH SSH 会重启并创建干净的默认 Profile。`,
    ...
    confirmRollbackBody: capturedAt => `将立即恢复 ${capturedAt} 创建的 Checkpoint 中的当前 Profile、settings.yaml 与 DSH home 补丁；重启后，DSH SSH 将使用回滚后的配置。`,
    confirmRollbackAction: '回滚',
    uninstalledSuccess: '插件已从当前 Profile 中卸载。请重启 DSH SSH 以使用更新后的插件配置。',
    rollbackSuccess: slotId => `已回滚到${slotId}。请重启 DSH SSH 以使用该配置；回滚后的第一次健康启动会保留现有三个槽位。`,
    profileSelectedSuccess: '已设为当前 Profile。请重启 DSH SSH 以使用该 Profile。',

  ...
  return {
    ...
    Menu: {
      buildFromTemplate: vi.fn((template: unknown[]) => {
        ...
        if (first?.label === 'DSH SSH' && Array.isArray(first.submenu)) {
...
describe('Electron desktop runtime', () => {
  beforeEach(() => {
    ...
    updater.filename.mockImplementation((platform: string, version: string) => (
      `DSH-SSH-${version}-${platform === 'darwin' ? 'mac.dmg' : 'windows.exe'}`
    ))
  ...
  it('shows native recovery when the renderer Loader reports a failed plugin', async () => {
    ...
    expect(electron.dialog.showMessageBox).toHaveBeenCalledWith(expect.objectContaining({
      ...
      buttons: ['Open DSH Terminal', 'Restart DSH SSH', 'Dismiss'],
    }))
  ...
  it('requires Desktop dialog confirmation before an ordinary restart', async () => {
    ...
    expect(restart).not.toHaveBeenCalled()
    expect(electron.dialog.showMessageBox).toHaveBeenCalledWith(expect.objectContaining({
      type: 'question', title: 'Restart DSH SSH', buttons: ['Restart', 'Cancel'], defaultId: 1, cancelId: 1,
    }))
  ...
  it('blocks the inherited update network and preserves confirmation-gated handoff', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('darwin')
    const response = Response.json({ version: '2.1.0' })
    electron.net.fetch.mockResolvedValueOnce(response)
    updater.download.mockResolvedValueOnce('/tmp/DSH-SSH-2.1.0-mac.dmg')
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})
    const release = runtime.schedule(spec)
    await runtime.mountScheduled()
    ...
    await expect(runtime.updates.request('https://www.dshdesktop.cn/api/desktop/version', { method: 'GET' }))
      .rejects.toThrow('DSH SSH 更新源尚未配置')
    ...
    expect(updater.download).toHaveBeenCalledWith({
      platform: 'darwin',
      version: '2.1.0',
      destinationPath: '/tmp/Downloads/DSH-SSH-2.1.0-mac.dmg',
      request: expect.any(Function),
      signal: controller.signal,
    })
    expect(electron.shell.openPath).toHaveBeenCalledWith('/tmp/DSH-SSH-2.1.0-mac.dmg')
    expect(updater.record).toHaveBeenCalledWith('/tmp/dsh-desktop-user-data', {
      platform: 'darwin',
      version: '2.1.0',
      path: '/tmp/DSH-SSH-2.1.0-mac.dmg',
    })
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(
      activeWindow,
      expect.objectContaining({
        title: 'DSH SSH Update Downloaded',
        buttons: ['OK'],
      }),
    )
  ...
  it('starts the downloaded Windows installer visibly before requesting orderly exit', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('win32')
    updater.download.mockResolvedValueOnce('C:\\Updates\\DSH-SSH-2.1.0-windows.exe')
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: 'C:\\Updates\\DSH-SSH-2.1.0-windows.exe',
    })
    ...
    expect(childProcess.spawn).toHaveBeenCalledWith(
      'C:\\Updates\\DSH-SSH-2.1.0-windows.exe',
      ['--updated', '--force-run'],
      {
        detached: true,
        stdio: 'ignore',
    ...
    expect(updater.record).toHaveBeenCalledWith('/tmp/dsh-desktop-user-data', {
      platform: 'win32',
      version: '2.1.0',
      path: 'C:\\Updates\\DSH-SSH-2.1.0-windows.exe',
    })
  ...
  it.each([
    ...
    const artifact = {
      ...
      version: '2.0.1',
      path: 'C:\\Updates\\DSH-SSH-2.0.1-windows.exe',
    }
  ...
  it('rejects a macOS handoff when the operating system cannot open the DMG', async () => {
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: '/tmp/DSH-SSH-2.1.0-mac.dmg',
    })

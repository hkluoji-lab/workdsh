...
| 官方能力 | [客户端模块](../../dsh-v0.1.6-alpha.2/subsystems/client-modules.zh.md)（`dsh.client` + `exports["./client"]` combo）；[Storage Domain](../../dsh-v0.1.6-alpha.2/subsystems/storage.zh.md)（`@deepseek-ai/dsh-storage-domain@0.1.6-alpha.2` 的 `defineDomain`/`domainTable`，拥有持久化与 schema 版本）；官方 Slot `main` 与 `sidebar.panellist`（`@deepseek-ai/dsh-client-ui-slots@0.1.6-alpha.2`，Sidebar/Workspace/Session owner 保持官方）；官方 Connection exact fetch（`@deepseek-ai/dsh-client-connection@0.1.6-alpha.2`，拥有 HTTP 路由与鉴权）；`@deepseek-ai/dsh-tools@0.1.6-alpha.2`（拥有工具注册、schema 与结果持久化）；`@deepseek-ai/cordis@4.0.2`（`ctx.effect`/`ctx.get` 拥有生命周期与可选依赖读取）。 |

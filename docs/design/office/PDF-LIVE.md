...
官方复用记录 PDF-01：沿用 docs/dsh-v0.1.7-alpha.2/subsystems/skills.md、tools.md、sidebar-right.zh.md，以及已验证 Office ContentService、官方 storageDomain/Connection/Sidebar Tab/present。业务差异是 PDF 页/元素数据及 PDF 编码器，不新增 Agent loop、文件服务器或模型路由。公共类型见 workdsh-contracts/office；接口在实现与验证后才进入工具定义。

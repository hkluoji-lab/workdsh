...
状态：首期拓扑明确；数据库实现归 D02 起各领域任务，当前 D01 尚未建立业务数据库。不是把内存或 Markdown 当作产品数据层。
...
D02 先实现基础身份/权限/审计的真实 DomainSpec、后端路由和迁移；D03—D09 各插件逐步加入其领域 Domain。D01 安装探针不等于业务持久化已经完成。

...
- 提交策略（用户 2026-09-25 裁决）：**与 alpha.1 批次合并提交**——`0.1.6-alpha.2 → 0.1.7-alpha.1` 与 `0.1.7-alpha.1 → 0.1.7-alpha.2` 的改动保持未提交，一并提交。**本批未提交、未推送、未发布、未改线上。**
...
日期：2026-09-25。本批为 0.1.7-alpha.1 批次的同族递进：依赖面与门禁全绿、官方变化面 13 条逐项裁定为「0 处需改代码」、文档镜像新增并完成路径重锚、探针与单测实测通过。**线上未动、未提交、未推送。** 剩余边界以「未覆盖项」表内说明为准，表中标记「未执行」的项不得计入已完成。

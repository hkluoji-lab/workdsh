...
| 官方能力 | 文档：`docs/dsh-v0.1.7-alpha.1/subsystems/subagent.md`、`tool-catalog.md`、`persistence-catalog.md`；锁定包 `@deepseek-ai/dsh-subagent` / `dsh-agent` / `dsh-llm`（0.1.5-rc.1）；公开入口：`ctx.subagents.registerProvider`（返回 Cordis effect disposer）、`ctx.agents.create`、`resolveChildDepth` / `resolveChildAgentOptions` / `childSessionMeta` / `captureDelegatedPolicyOverrides` / `appendDelegatedPolicyOverrides` / `finalAssistantOutput`、`createMessage`、`ctx.tools.register`、`agent/pre-step`、既有 `ctx.workdshExperts` 与 `team-sop.ts` 纯策略 |
...
| 官方能力 | 文档：`docs/dsh-v0.1.7-alpha.1/tool-catalog.md`（present/subagent）、`persistence-catalog.md`（deliverables/presented）、`subsystems/subagent.md`；锁定包 `@deepseek-ai/dsh@0.1.5-rc.1` 同版本组件；公开入口：`ctx.tools.register/guard`、`SubagentRuntime.start`、`AgentRegistry.create`、`Session.snapshotEvents`、`StorageDomain`、`agent/pre-step` 与现有 `ctx.workdshExperts` 契约 |
...
| 项目 | 本轮范围 |
|---|---|
| 官方文档 | docs/dsh-v0.1.7-alpha.1/subsystems/agent-team.md、subagent.md；锁定发布包公开 exports/types |

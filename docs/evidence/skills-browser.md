...
官方文档：../dsh-v0.1.7-alpha.1/subsystems/skills.zh.md。版本 rc.1；公开 dsh-api-remotes/client 的 remote.session.list、remote.skills.list 与 dsh-api-session-controller/client 的 sessions.open。现有 Remote 与 Session 技能消费证据见 d01-client.md、d01-presets.md。
...
| 官方能力 | `../dsh-v0.1.7-alpha.1/subsystems/slots.zh.md`（`ctx.slots.inject/register`、main 面板贡献与生命周期）；`../dsh-v0.1.7-alpha.1/subsystems/web-client.zh.md`（布局与面板选择）。锁定包 `@deepseek-ai/dsh-client-ui-slots@0.1.5-rc.1`（`PropsRuntime`/`InjectFace`）；面板注册键 `workdsh-skills` 与 URL 呈现（`workdsh-view=skills`）继续由 bundle `NavigationLocation` 承担。 |

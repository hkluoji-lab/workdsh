...
| 官方能力 | 文档：`docs/dsh-v0.1.7-alpha.1/subsystems/skills.md`（技能子系统与资源基地址）；锁定包 `@deepseek-ai/dsh-skill` 0.1.5-rc.1（`SkillRegistration`：`content` 为静态 string、注册同步、返回 Cordis effect disposer；`SkillResourceBase { kind: 'directory', path }`）、`ctx.effect` 生命周期托管；`dsh-skill-filesystem` 的 frontmatter 惯例（`name`/`description`/`when-to-use`）作格式对照 |

...
- 打包与安装（`docs/dsh-v0.1.7-alpha.2/user/develop/basic/publish.zh.md`）：组合包贡献配置层；无 `dsh.bundle` 的包只作普通依赖、不激活层；制品解析是激活前提。
- 组合与 HMR（`docs/dsh-v0.1.7-alpha.2/cordis-tutorial/06-composition-and-hmr.zh.md`）：`inject` 无提供方时 Fiber 静默 PENDING（合法态）；经 `ctx.registry` + `FiberState.PENDING` 诊断。
- 添加包（`docs/dsh-v0.1.7-alpha.2/cookbook/adding-a-package.zh.md`）：`src/index.ts` 可为 service default export 或 plugin（name/inject/apply/Config）。

...
日期：2026-09-16，2026-09-24 用户批准实施。状态：**已批准；首期切片已实现（模块 `workdsh-plugin-assistant@0.1.0-alpha.1`）**。D00—D15 门槛含义不变，D16 的步骤状态仍按依赖就绪判定。
...
用户指令「网站左侧栏中助理待开发，请分析并开发实现可用」。按本节决定实现首期切片，设计与复用记录见 [助理模块设计](../design/assistant/README.md)。

- 已实现：对象与只追加修订、引用解析（技能／专家／连接器实例）、列表与详情／编辑分离、归档与恢复、按主体与组织隔离。
- 沿本条决定的收敛：`触发方式` 在首期只是修订中的记录字段，不注册调度器、不建立入站端点；定时执行归 D12 自动化，入站机器人服务仍待另行 ADR。
- 「待决事项」第 2 条（首期触发方式如何落在官方调度与审批上）已按上述收敛处理，不在本模块实现调度。

## 未执行

首期未验证：真实模型执行与端到端交付、定时触发、外部消息入站、多主体多组织端到端鉴权（属 D14 企业阶段）。

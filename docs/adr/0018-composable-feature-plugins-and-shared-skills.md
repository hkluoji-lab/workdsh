...
采用官方的插件组合体系：Harness 运行底座本身由官方插件组合，WorkDSH 业务能力继续通过同一套 Cordis 服务、生命周期、Loader 与 Profile 扩展。不在其上建立一个承载全部业务的 WorkDSH 大核心，也不另造插件管理框架。所有业务功能插件化；共享库和用户创作的业务对象保持各自类型。

默认 WorkDSH 组合只是产品选用哪些官方与自有插件、怎样配置它们的声明。它不能通过直接调用 applyX(ctx) 将各功能的服务、工具和页面隐藏挂在一个总入口中；需要嵌套时也必须使用官方插件注册与生命周期。
...
| 默认组合 | WorkDSH bundle/Profile 明确装配已实现的功能，提供整套安装体验；不隐藏调用各功能初始化 helper，不拥有其业务实现 |
| 公共库 | contracts、ui 提供类型、校验及展示组件；不为统一外观而声明可运行插件入口 |
| 业务对象 | 专家、专家团、具体 Skill、连接实例是插件管理的数据；创建一个专家不产生一个 npm 包 |

```mermaid
flowchart TB
  P["WorkDSH 默认 Profile / bundle"] -. "装配" .-> E["专家插件"]
  P -. "装配" .-> S["技能管理插件"]
  P -. "装配" .-> C["连接器等功能插件"]
  E -->|"公开契约：解析依赖、保留修订"| S
...
WorkDSH 管理入口的依赖检查不能替代官方 CLI 行为。用户直接改 Profile 或用 CLI 移除依赖后，启动与任务入口必须能报告缺失；不能声称 npm 会自动启用所有依赖层或阻止业务依赖被移除。
...
- [Skill 官方说明本地镜像](../dsh-v0.1.7-alpha.2/subsystems/skills.zh.md)：原生 host/scope 分层目录；实际适配仍须在锁定发布包上验证。

...
- 状态（2026-09-24 用户批准实施）：首期切片已实现为独立插件 `workdsh-plugin-assistant@0.1.0-alpha.1`，范围即上述「首期范围」；设计与官方能力复用记录见 [助理模块设计](design/assistant/README.md)。D16 步骤状态仍为 `todo`（依赖 D08 未完成），与 D05/D06/D07「模块已实现、步骤未验收」同口径。触发方式首期只作为修订字段记录，调度归 D12、入站机器人服务另立 ADR。

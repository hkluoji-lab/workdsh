...
export function installEditor(api: <T>(op:string,data:unknown)=>Promise<T>, getId:()=>string) {
  ...
  let generation=0
  const canClose=(id:string)=>{const d=documents.get(id);return !d || (!d.busy && (d.text===d.saved || confirm('放弃 '+d.path+' 的未保存修改？')))}
  function refresh(){const d=documents.get(getId());panel.hidden=!d;if(!d)return;input.value=d.text;input.disabled=d.busy;label.textContent=d.path+(d.text!==d.saved?' ●':'');save.disabled=d.busy||d.text===d.saved;status.textContent=''}
  ...
  panel.querySelector<HTMLButtonElement>('[data-close]')!.onclick=()=>{if(canClose(getId())){documents.delete(getId());++generation;refresh()}}

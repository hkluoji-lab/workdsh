/** DSH SSH Host plugin: owns the selected native shell generation. */
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (runtime === undefined) {
    process.stderr.write(
      'dsh-plugin-desktop: this profile is composed with the DSH SSH shell, which requires the desktop launcher (desktopRuntime).\n'
      + 'Start it with `dsh-desktop`, or select this profile inside the packaged DSH SSH application.\n'

...
function updateAvailableNotification(locale: DesktopLocale, version: string): DesktopNotification {
  return locale === 'zh'
    ? { title: 'DSH SSH 有可用更新', body: `版本 ${version} 已可下载。打开 DSH SSH 即可继续。` }
    : { title: 'DSH SSH Update Available', body: `Version ${version} is ready to download. Open DSH SSH to continue.` }
}

/** Headless-safe npm launcher for the DSH SSH Electron executable. */
...
Launch DSH SSH with the selected Web-capable profile.
...
export function defaultDesktopUserDataDirectory(
  ...
  if (platform === 'win32') {
    ...
    if (appData === undefined || appData.length === 0) {
      throw new Error('APPDATA is unavailable; cannot locate DSH SSH diagnostics')
    }
    return path.join(appData, 'DSH SSH')
  }
  if (platform === 'darwin') return path.join(homeDirectory, 'Library', 'Application Support', 'DSH SSH')
  const config = environment.XDG_CONFIG_HOME
  return path.join(config === undefined || config.length === 0 ? path.join(homeDirectory, '.config') : config, 'DSH SSH')
}
...
async function launchElectron(): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(
      ...
      + 'Or use the packaged DSH SSH application.\n',
    )
    return 1
  }

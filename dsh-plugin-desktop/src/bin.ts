/** Headless-safe npm launcher for the WorkDSH Electron executable. */
...
Launch WorkDSH with the selected Web-capable profile.
...
export function defaultDesktopUserDataDirectory(
  ...
  if (platform === 'win32') {
    ...
    if (appData === undefined || appData.length === 0) {
      throw new Error('APPDATA is unavailable; cannot locate WorkDSH diagnostics')
    }
    return path.join(appData, 'WorkDSH')
  }
  if (platform === 'darwin') return path.join(homeDirectory, 'Library', 'Application Support', 'WorkDSH')
  const config = environment.XDG_CONFIG_HOME
  return path.join(config === undefined || config.length === 0 ? path.join(homeDirectory, '.config') : config, 'WorkDSH')
}
...
async function launchElectron(): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(
      ...
      + 'Or use the packaged WorkDSH application.\n',
    )
    return 1
  }

...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `DSH-SSH-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'DSH SSH.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing DSH SSH.exe: ${portablePath}`)
  }

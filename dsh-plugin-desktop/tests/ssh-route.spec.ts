...
describe('authenticated SSH carrier', () => {
  it('rejects unauthenticated and cross-origin actions before connecting', async () => {
    ...
    try {
      expect((await request(denied,'/desktop/ssh/probe','POST',{})).code).toBe(403)
      expect((await request(allowed,'/desktop/ssh/probe','POST',{},'https://evil.example')).code).toBe(403)
      expect((await request(allowed,'/desktop/ssh/connect','POST',{})).code).toBe(400)
  ...
  it('redirects the asset base and rejects resource traversal and unsupported methods', async () => {
    const f=fixture()
    try {
      const redirect=await request(f,'/desktop/ssh','GET');expect(redirect.code).toBe(302);expect(redirect.headers.location).toBe('/desktop/ssh/')
      expect((await request(f,'/desktop/ssh/','GET')).text).toContain('ssh-root')
      expect((await request(f,'/desktop/ssh/assets/foo%2fbar.js','GET')).code).toBe(404)
      expect((await request(f,'/desktop/ssh/list','GET')).code).toBe(405)
      expect((await request(f,'/desktop/ssh/list','POST',{id:'unknown',path:'/'})).code).toBe(400)
    } finally { f.dispose() }
  })
})

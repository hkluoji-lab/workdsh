...
export async function exportAndPresent(
  ...
  if (bytes.length > maxBytes) throw new Error(`${format} 导出超过 ${isPpt ? 8 : 1} MiB 上限。`);
  const title =
  ...
  try { write = await ctx.tools.execute({
    ...
    arguments: {
      ...
      description: `Export current document as ${format}`,
      timeoutMs: 30000,
    },
  ...
  if (
    ...
    throw new Error(
      `${format} 文件写入失败、被策略拒绝或结果未知，未交付文件。目标 ${path}；可使用同一修订 ${snapshot.revision} 重试导出并核对已有文件，或使用右侧下载。`,
    );
  exec.signal.throwIfAborted();
  let present;

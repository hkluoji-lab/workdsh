...
export function officeInputSources(office: OfficeClient, currentSession: () => string | undefined): InputTriggerSource[] {
  return [{
    ...
    candidates: async (_session, req) => {
      ...
      return officeTypes.filter(([id, label]) => !filter || id.includes(filter) || label.toLowerCase().includes(filter)).map(([id, label, live]) => ({
        ...
        description: `${label} · 新建${live ? " · 支持实时写作" : " · 实时编辑待接入"}`,
        icon: "file" as const,
      }));
    },
    onPick: ({candidate}) => {
    ...
    codec: {
      ...
      serialize: async (ref, signal) => {
        ...
        return `\n[用户选择的 Office 输出意图]\n${JSON.stringify({version: 1, outputType: id, label, defaultAction: "create", targetRequired: false, liveEditingAvailable: live})}\n没有明确标注为 target 的文档引用时，创建新文档；reference 引用仅作资料，保留原件。明确 target 时修改该对象，多个 target 或冲突输出类型需先澄清。Word 使用 content_open/content_read/content_edit/content_present，创建后立即打开右侧，分批写入。其余类型实时适配尚未完成，不得当作 Word 创建或宣称右侧实时编辑已支持；按所选类型生成可用文件，能力不足明确说明。\n`;
      },
    },
  }, {

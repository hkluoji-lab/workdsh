...
export function LibraryPanel({ management, previewRegistry, toggleNavigation, currentSessionId }: Props) {
  ...
  const createFolder = async (parentId?: string) => { const name = window.prompt('文件夹名称'); if (!name) return; try { await management.createFolder(name, parentId); await loadTree(); } catch (cause) { setError(cause instanceof Error ? cause.message : '新建文件夹失败。'); } };
  const createText = async (kind: 'markdown' | 'text', parentId?: string) => { const extension = kind === 'markdown' ? '.md' : '.txt'; const name = window.prompt('文件名称', kind === 'markdown' ? '未命名文档.md' : '未命名文本.txt'); if (!name) return; const finalName = name.toLowerCase().endsWith(extension) ? name : `${name}${extension}`; try { await management.importFile(new File([kind === 'markdown' ? `# ${finalName.slice(0, -extension.length)}\n` : '\n'], finalName, { type: kind === 'markdown' ? 'text/markdown' : 'text/plain' }), parentId); await loadTree(); } catch (cause) { setError(cause instanceof Error ? cause.message : '新建资料失败。'); } };
  ...
  return <section className="wd-library"><style>{libraryCss}</style>
    ...
    <main className="wd-library-content">{notice ? <div className="wd-library-toast success">{notice}</div> : null}{error ? <div className="wd-library-toast error">{error}</div> : null}
      {selected ? <><header className="wd-library-document-header"><div><span>我的资料</span><b>/</b><strong>{selected.name}</strong>{selected.asset?.status === 'disabled' ? <em>已停用</em> : selected.revision?.conversionStatus === 'failed' ? <em>转换失败</em> : selected.revision?.conversionStatus === 'pending' ? <em>转换中</em> : null}</div><div className="wd-library-head-actions">{isEditable(selected) ? <button onClick={() => void beginEdit()}>编辑</button> : null}<button onClick={() => void downloadOriginal()}>下载</button><button onClick={event => showMenu(event, { entry: selected })}>•••</button></div></header><div className="wd-library-document-body">{selected.revision?.conversionStatus === 'failed' ? <div className="wd-library-conversion-warning">原件已安全保存，但检索文本转换失败，因此暂不可搜索。仍可预览或下载原件。</div> : null}{draft ? <div className="wd-library-editor"><div className="wd-library-editor-bar"><strong>编辑草稿</strong><span>发布后生成新的只读修订</span><button onClick={() => setDraft(undefined)}>取消</button><button className="primary" onClick={() => void publishDraft()}>发布新版本</button></div><div className="wd-library-diff"><section><b>当前修订</b><pre>{draft.content}</pre></section><section><b>草稿预览</b><pre>{draftContent}</pre></section></div><textarea aria-label="草稿内容" value={draftContent} onChange={event => setDraftContent(event.currentTarget.value)} /></div> : selected.asset?.status === 'disabled' ? <div className="wd-library-state">这份资料已停用。重新启用后才能查看或用于任务。</div> : originalUrl ? <iframe className="wd-library-pdf" title={selected.name} src={originalUrl} /> : officePreview ? <div ref={officeHost} className="wd-library-office-preview" /> : <article className={`wd-library-original ${selected.asset?.kind ?? ''}`}><div className="wd-library-meta">修订 {selected.revision?.number ?? 1} · {selected.asset?.kind === 'docx' || selected.asset?.kind === 'pptx' ? 'Office 原始预览不可用，显示检索文本' : '原始内容'}</div><pre>{content}</pre></article>}</div></>
      : view === 'search' ? <div className="wd-library-list-view"><h2>搜索</h2><form onSubmit={event => { event.preventDefault(); void runSearch('search'); }}><input autoFocus value={query} onChange={event => setQuery(event.currentTarget.value)} placeholder="搜索资料名称和内容"/><button>搜索</button></form><div className="wd-library-filters"><select aria-label="资料类型" value={searchKind} onChange={event => setSearchKind(event.currentTarget.value as LibraryAssetKind | '')}><option value="">全部类型</option><option value="markdown">Markdown</option><option value="text">TXT</option><option value="pdf">PDF</option><option value="docx">Word</option><option value="pptx">PowerPoint</option></select><select aria-label="资料来源" value={searchSource} onChange={event => setSearchSource(event.currentTarget.value as LibraryAsset['source'] | '')}><option value="">全部来源</option><option value="upload">上传导入</option><option value="created">新建资料</option><option value="task">本地产物</option></select><label>更新日期从 <input type="date" value={searchAfter} onChange={event => setSearchAfter(event.currentTarget.value)} /></label></div>{busy ? <div className="wd-library-state">正在搜索…</div> : <ResultList rows={resultEntries} hits={hits} open={open} />}</div>
    ...
    {menu ? <div className="wd-library-popover" style={{ left: menu.x, top: menu.y }} onClick={event => event.stopPropagation()}>{menu.create ? <><button onClick={() => void createText('markdown', menu.parentId)}>新建文档（.md）</button><button onClick={() => void createText('text', menu.parentId)}>新建文本（.txt）</button><button onClick={() => void createFolder(menu.parentId)}>新建文件夹</button><hr/><button onClick={() => { uploadParent.current = menu.parentId; upload.current?.click(); }}>上传和导入</button></> : menu.entry ? <>{menu.entry.kind === 'folder' ? <><button onClick={event => showMenu(event, { create: true, parentId: menu.entry!.id })}>在此新建或导入</button><hr/></> : null}<button onClick={() => void addToTask(menu.entry!)}>添加到当前任务</button><hr/><button onClick={() => beginMove(menu.entry!)}>移动到…</button><button onClick={() => void rename(menu.entry!)}>重命名</button>{menu.entry.asset ? <button onClick={() => void setStatus(menu.entry!)}>{menu.entry.asset.status === 'disabled' ? '重新启用' : '停用'}</button> : null}<button className="danger" onClick={() => void remove(menu.entry!)}>删除</button></> : null}</div> : null}
...
function ResultList({ rows, hits, open }: { rows: readonly LibraryTreeEntry[]; hits: readonly LibrarySearchHit[]; open: (row: LibraryTreeEntry) => Promise<void> }) {
  const typeName = { markdown: 'Markdown', text: 'TXT', pdf: 'PDF', docx: 'Word', pptx: 'PowerPoint' } as const;

...
export function LibraryPanel({ management, previewRegistry, toggleNavigation, currentSessionId }: Props) {
  ...
  const addToTask = async (entry: LibraryTreeEntry) => { const sessionId = currentSessionId(); if (!sessionId) { setError('请先打开一个对话，再添加资料。'); return; } try { const current = await management.taskSelection(sessionId); await management.setTaskSelection(sessionId, [...new Set([...current.map(row => row.nodeId), entry.id])]); notifyLibrarySelectionChanged(sessionId); setMenu(undefined); setNotice(`已将“${entry.name}”放入当前对话输入框。`); } catch (cause) { setError(cause instanceof Error ? cause.message : '添加到任务失败。'); } };

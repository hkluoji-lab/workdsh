  ...
  const unregister = connection.fetch.register({
    ...
    fetch: async request => {
      try {
        ...
        const messages: Record<string, string> = { 'library/not-found': '资料不存在或无权访问。', 'library/name-conflict': '同一目录已有同名项目。', 'library/unsupported-format': '当前仅支持 Markdown、TXT、PDF、DOCX 和 PPTX。', 'library/file-size': '文件为空或超过 50 MB。', 'library/cycle': '文件夹不能移动到自己的子目录。', 'library/invalid-name': '资料名称无效。' };

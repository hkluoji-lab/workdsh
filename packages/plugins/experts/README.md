...
一个插件管理多个专家与专家团配置。当前源码候选为 `workdsh-plugin-experts@0.1.0-alpha.9`，面向 **DeepSeek Harness 0.1.7-alpha.2 / Cordis 4.0.4**；候选代码与已经发布的安装包分别验收。
...
新候选依赖 DSH 0.1.7-alpha.2，应使用同一次构建产出的配套 tgz；不要把旧 Release 的安装包当作已含本次迁移。先在独立 Profile 验证，再部署实际使用的 Profile：

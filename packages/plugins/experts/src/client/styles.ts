 ...
 * sibling Skill panel. Colours follow the WorkDSH dark tokens registered by the bundle
 * theme and match the delivered Skill panel so the shared capability center reads as one
 * surface. Responsive: 4 columns → 3 → 2 → 1; detail dialog capped at 800px, editor at 760px,
 * `max-height: min(820px, 100dvh - 80px)`, full-screen panel under 600px.
 */
export const expertsCss = `${modalCss}
.wd-experts{height:100%;overflow:auto;background:#121212;color:#e7e7e7;font:14px/22px "PingFang SC","Microsoft YaHei",sans-serif;padding:24px;box-sizing:border-box}
...
.wd-experts button{cursor:pointer;border:1px solid #343434;border-radius:8px;background:#242424;min-height:36px;padding:6px 12px}
.wd-experts button:hover:not(:disabled){background:#383838}
.wd-experts button:disabled{cursor:not-allowed;color:#777}
.wd-experts input,.wd-experts select,.wd-experts textarea{border:1px solid #343434;border-radius:8px;background:#1b1b1b;min-height:36px;padding:6px 12px}
.wd-experts :focus-visible{outline:2px solid #ddd;outline-offset:3px}
.wd-experts .muted{color:#a5a5a5}
...
.wd-experts .cap-tab.active{background:#343434}
.wd-experts .cap-tab:disabled{cursor:default;color:#aaa}
...
.wd-experts .mine-toggle.active{background:#343434;border-color:#4a4a4a}
.wd-experts .create-expert{background:#eeeeee;color:#171717;border-color:#eeeeee;font-weight:600}
...
.wd-experts .filter-tabs button:disabled{color:#666}
.wd-experts .counts{font-size:12px;margin:0 0 16px;color:#a5a5a5}
.wd-experts .counts.error{color:#ffb4ab}
.wd-experts .grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}
.wd-experts .card{position:relative;min-width:0;border-radius:17px;border:1px solid #292929;background:#242424;min-height:150px;padding:24px;display:flex;flex-direction:column;gap:12px}
.wd-experts .card:hover{border-color:#3b3b3b;background:#282828}
...
.wd-experts .badge.warn{color:#ffcf8f;border-color:#5b4a2c}
.wd-experts .badge.bad{color:#ff9b95;border-color:#765349}
.wd-experts .pin{margin-left:auto;background:transparent;border:0;min-height:28px;padding:2px 6px;font-size:15px;color:#888}
.wd-experts .pin.on{color:#ffd479}
.wd-experts .more-button{width:34px;min-height:30px;padding:0;border:0;font-size:18px;background:transparent;color:#bbb}
.wd-experts .card-actions{position:relative}
.wd-experts .card-menu{position:absolute;z-index:20;right:0;top:calc(100% + 6px);width:172px;padding:8px;background:#242424;border:1px solid #424242;border-radius:14px;box-shadow:0 14px 36px #0008}
...
.wd-experts .card-menu button:hover:not(:disabled){background:#333}
.wd-experts .card-menu .danger{color:#ff8f8f}
...
.wd-experts .skeleton{border-radius:17px;border:1px solid #292929;background:linear-gradient(100deg,#1e1e1e 30%,#262626 50%,#1e1e1e 70%);background-size:200% 100%;animation:wd-shimmer 1.3s infinite;min-height:150px}
@keyframes wd-shimmer{to{background-position:-200% 0}}
.wd-experts .notice{display:flex;gap:10px;align-items:flex-start;padding:12px 14px;border-radius:12px;border:1px solid #343434;background:#1d1d1d;margin:0 0 16px}
.wd-experts .notice.error{border-color:#765349;background:#2a211f}
.wd-experts .notice.warn{border-color:#5b4a2c;background:#241f16}
.wd-experts .notice.info{border-color:#2c5b53;background:#182422}
...
.expert-dialog .summon{background:#eeeeee;color:#171717;border-color:#eeeeee;font-weight:600}
.expert-dialog button{font:inherit;cursor:pointer;min-height:38px;padding:7px 15px;border:1px solid #414141;border-radius:9px;background:#2b2b2b;color:#e8e8e8}
...
.expert-dialog .detail-desc{white-space:pre-wrap;overflow-wrap:anywhere;color:#ddd;line-height:1.7;margin:0}
.expert-dialog .tag-row{display:flex;flex-wrap:wrap;gap:8px;margin:16px 0 0}
.expert-dialog .tag{font-size:12px;padding:3px 10px;border-radius:999px;background:#1d1d1d;border:1px solid #3a3a3a;color:#cfcfcf}
.expert-dialog .example-list{display:grid;gap:10px}
.expert-dialog .example{display:flex;align-items:center;gap:12px;width:100%;text-align:left;padding:14px 16px;border:1px solid #343434;border-radius:12px;background:#1e1e1e}
.expert-dialog .example:hover:not(:disabled){background:#262626;border-color:#454545}
...
.expert-dialog .example .example-text span{color:#a5a5a5;font-size:14px;line-height:1.65;white-space:pre-wrap;overflow-wrap:anywhere}
.expert-dialog .cap-list{display:grid;gap:8px}
.expert-dialog .cap-row{display:flex;align-items:center;gap:10px;padding:10px 14px;border:1px solid #303030;border-radius:10px;background:#1c1c1c}
...
.confirm-dialog .primary{background:#eee;color:#171717;border-color:#eee;font-weight:600}
.confirm-dialog .danger.solid{background:#d94b4b;border-color:#d94b4b;color:#fff}
.confirm-dialog .danger.solid:disabled{background:#633b3b;border-color:#633b3b;color:#aaa;cursor:not-allowed}
.confirm-dialog .digest{font:12px/18px ui-monospace,SFMono-Regular,Menlo,monospace;color:#9aa;word-break:break-all}
.confirm-dialog .diff-list{margin:0;padding-left:20px;color:#ccc;line-height:1.9}
...
.publish-dialog .confirm-actions{flex-shrink:0;padding-top:16px;border-top:1px solid #343434}
.publish-dialog .confirm-actions .primary{background:#e5e5e5;color:#202020;border-color:#e5e5e5}
.publish-dialog .confirm-actions .primary:hover:not(:disabled){background:#d5d5d5;color:#202020}
...
.publish-dialog .preview-avatar{flex-shrink:0;display:grid;place-items:center;width:56px;height:56px;border-radius:50%;background:#413653;color:#d2c1ec;font-size:24px}
.publish-dialog .usage-preview h3{font-size:15px;margin:20px 0 8px;color:#e7e7e7}
...
.publish-dialog .preview-tags span{border:1px solid #343434;border-radius:16px;padding:3px 10px;color:#a5a5a5;font-size:12px}
.publish-dialog .preview-example{border:1px solid #343434;border-radius:12px;padding:12px 14px;margin:10px 0;background:#242424}
.publish-dialog .preview-example p{margin:4px 0 0}
.publish-dialog .usage-preview ul{margin:8px 0;padding-left:20px;color:#ccc}
...
.editor-dialog .editor-foot{display:flex;align-items:center;gap:12px;padding:16px 28px;flex-shrink:0;border-top:1px solid #2c2c2c;background:#1a1a1a}
.editor-dialog .editor-foot .saved-at{color:#a5a5a5;font-size:12px;margin-right:auto}
.editor-dialog .group{border:1px solid #2c2c2c;border-radius:14px;padding:20px 22px;margin-bottom:18px;background:#1a1a1a}
...
.editor-dialog .field .hint{color:#888;font-size:12px;margin-top:4px}
.editor-dialog .field .field-error{color:#ff9b95;font-size:12px;margin-top:4px}
.editor-dialog input,.editor-dialog textarea{box-sizing:border-box;width:100%;min-width:0;background:#242424;color:#eee;border:1px solid #414141;border-radius:9px;padding:10px 12px;font:inherit}
.editor-dialog input:focus-visible,.editor-dialog textarea:focus-visible{outline:2px solid #8ab4ff;outline-offset:2px}
.editor-dialog .tag-editor{grid-template-columns:repeat(2,minmax(0,1fr))}
.editor-dialog .example-editor-row{display:grid;gap:10px;padding:16px;border:1px solid #353535;border-radius:12px;background:#212121}
.editor-dialog .example-editor-row textarea{min-height:100px;resize:vertical;line-height:1.65}
.editor-dialog .example-editor-row .remove{justify-self:end;color:#ff9b95}
...
.editor-dialog .field .counter{float:right;color:#777;font-size:12px;font-weight:400}
.editor-dialog .field.invalid input,.editor-dialog .field.invalid textarea{border-color:#a5564f}
...
.editor-dialog .list-row .remove{flex:none;min-height:36px;padding:0 12px;color:#ff9b95}
.editor-dialog .add-row{justify-self:start;background:transparent;border:1px dashed #4a4a4a;color:#ccc}
.editor-dialog button{font:inherit;cursor:pointer;min-height:38px;padding:7px 15px;border:1px solid #414141;border-radius:9px;background:#2b2b2b;color:#e8e8e8}
.editor-dialog button:disabled{cursor:not-allowed;opacity:.5}
.editor-dialog .primary{background:#eee;color:#171717;border-color:#eee;font-weight:600}
.editor-dialog .conflict{border:1px solid #5b4a2c;background:#241f16;border-radius:12px;padding:16px 18px;margin-bottom:18px}
.editor-dialog .conflict h4{margin:0 0 8px;color:#ffcf8f}
.editor-dialog .conflict .conflict-actions{display:flex;gap:10px;margin-top:12px}
.editor-dialog .issues{border:1px solid #765349;background:#2a211f;border-radius:12px;padding:14px 18px;margin-bottom:18px}
.editor-dialog .issues h4{margin:0 0 8px;color:#ffb4ab}
.editor-dialog .issues ul{margin:0;padding-left:20px;color:#ddd;line-height:1.9}
.editor-dialog .equipped-row{display:flex;align-items:center;gap:12px;padding:10px 12px;border:1px solid #353535;border-radius:10px;background:#242424}
.editor-dialog .equipped-row strong{flex:1;min-width:0;overflow-wrap:anywhere}
.editor-dialog .equipped-row .remove{color:#ff9b95}
.editor-dialog .skill-picker{margin-top:16px;padding:16px;border:1px solid #494949;border-radius:12px;background:#242424}
...
.editor-dialog .skill-choice{display:flex;align-items:flex-start;gap:12px;padding:12px;border:1px solid #414141;border-radius:10px;cursor:pointer}
.editor-dialog .skill-choice input{width:18px;height:18px;flex:none;padding:0;margin:3px 0;accent-color:#20bba6}
.editor-dialog .skill-choice>span{flex:1;min-width:0;overflow-wrap:anywhere}
.editor-dialog .skill-choice small,.expert-dialog .skill-copy small{display:block;color:#aaa;line-height:1.6;margin-top:4px}
...
.import-dialog .dropzone:hover,.import-dialog .dropzone.dragging{border-color:#9a9a9a;background:#262626}
.import-dialog .dropzone:disabled{cursor:wait;opacity:.7}
.import-dialog .upload-glyph{width:50px;height:42px;display:grid;place-items:center;border:2px solid #8e8e8e;border-radius:8px;font-size:24px;color:#aaa}
.import-dialog .dropzone strong{font-size:17px;font-weight:500}
.import-dialog .dropzone small{color:#999}
...
.import-dialog dt{color:#929292}
.import-dialog dd{margin:0;overflow-wrap:anywhere}
.import-dialog .issue-list{margin:8px 0 0;padding-left:20px;color:#ffb4ab;line-height:1.9}
.import-dialog .import-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:22px}
.import-dialog .import-actions button{min-height:42px;padding:8px 18px;border:1px solid #454545;border-radius:9px;background:#292929;cursor:pointer}
.import-dialog .import-actions .install{background:#eee;color:#171717;border-color:#eee;font-weight:600}
.import-dialog .error{color:#ffb4ab;margin:16px 0 0}

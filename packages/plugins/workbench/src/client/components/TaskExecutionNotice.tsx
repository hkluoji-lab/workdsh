...
export function TaskExecutionNotice({ useSession, useProjection, useConversation }: PropsRuntime<'conversation.input.dock'>) {
  ...
  if (!text) return null;
  return <div role="status" data-testid="workdsh-task-execution-notice" style={{ padding: '6px 10px', fontSize: 12, color: 'var(--dsw-alias-label-secondary)', lineHeight: 1.5 }}>{text}</div>;
}

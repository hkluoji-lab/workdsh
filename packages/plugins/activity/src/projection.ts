...
export function projectActivity(entries: readonly Entry[], running: boolean): ActivityState {
  ...
  for (const {event} of entries) {
    ...
    if (event.type === 'tool/result') {
      // Session format V4: the result is a first-class tool-role message, so the answered
      // call and its outcome live on the message itself (`toolCallId` / `isError`) instead
      // of a wrapped `tool-result` content block.
      const message = record(data.message);
      const callId = String(message.toolCallId);
      if (skills.has(callId) && message.isError !== true) skill = skills.get(callId);

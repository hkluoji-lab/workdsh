...
// Launch the CLI installed inside the Profile, not the workspace copy: the
// launcher and the ConfigEditor share module-local dsh-app-boot registration,
// and only the Profile-local CLI resolves the same instance as the installed
// Profile packages.
...
let installed;
try { installed = JSON.parse(readFileSync(resolve(profile, 'node_modules/@deepseek-ai/dsh/package.json'), 'utf8')); }
